...
import ChildWindow.RepairPlanFrame.Table_Model;
import CustomComponents.BrowserFrame;
import CustomComponents.ReadWebsite;

public class UnfinishedRepairFrame extends JFrame {
	...
	public UnfinishedRepairFrame(JFrame daddy)
	{
		father = daddy;
		self = this;
		setVisible(true);
	    			...
	    			if(table.getSelectedColumn() == 10)
	    			{
	    				if(table.getValueAt(table.getSelectedRow(), 0).equals("发动机"))
	    				{
	    					try {
		    					...
		    					BrowserFrame a = new BrowserFrame(self, web.get(6));
	    				...
	    				else if(table.getValueAt(table.getSelectedRow(), 0).equals("油箱"))
	    				{
	    					try {
		    					ReadWebsite readWebsite = new ReadWebsite();
		    					ArrayList<String> web = readWebsite.getWebsite();
		    					BrowserFrame a = new BrowserFrame(self, web.get(7));
		    					self.setVisible(false);
							} catch (Exception e2) {
								// TODO: handle exception
							}
	...
	public static void main(String[] args) {
		...
		UnfinishedRepairFrame a = new UnfinishedRepairFrame(daddy);
	}
	...
	class Table_Model extends AbstractTableModel {
	    ...
	    private String[] title_name = { "产品名称", "维修计划", "维修级别", "维修部位", "维修要求", "维修方式", "维修备件", "维修资源", "注意事项","完成状态确认","查看"};
