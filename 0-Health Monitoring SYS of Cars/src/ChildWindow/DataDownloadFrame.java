...
public class DataDownloadFrame extends JFrame {

	private JFrame father;
	private JPasswordField passwordField;
	
	public  DataDownloadFrame(JFrame daddy)
	{
		father = daddy;
		...
		//设置界面上半部分
		JLabel download = new JLabel("下载数据包");
		download.setFont(new Font("宋体",Font.PLAIN, 20));
		box1.add(download);
		box1.add(Box.createVerticalStrut(10));
		
		JLabel destination = new JLabel("下载到");
		...
		downloadField.addItem("运行状态数据包");
		downloadField.addItem("故障报告数据包");
		downloadField.addItem("设备状态数据包");
		downloadField.addItem("自检状态数据包");
		...
		destinationField.addItem("动力自动诊断设备");
		...
		confirmButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				JOptionPane.showMessageDialog(null, "数据下载成功");
			}
		});
		add(confirmButton);
	...
	public static void main(String[] args) {
		...
		DataDownloadFrame a = new DataDownloadFrame(daddy);
